...
class R2Interface:
    def __init__(self, serial_port):
        self.connection =  serial.Serial(port = serial_port, baudrate=19200, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE,
            stopbits=1, xonxoff=False, rtscts=True, timeout=2)
    ...
    def send_command(self, command):
        ...
        time.sleep(0.1)
        response = self.connection.readline()
        print(f"Sent: {command.strip()} | Received: {response.decode(errors='ignore').strip()}")
        return response
...
# def simple_flow_sweep(r2: R2Interface, F0_mL_per_min, F1_mL_per_min, pickup_vol_mL=0.1, reactor_vol_mL=10):
...
if __name__ == "__main__":
    ...
    print(R2S.send_command("GA"))  # Get pump A status
    print(R2S.send_command("GB"))  # Get pump B status
    print(R2S.send_command("GP"))  # General pump info
    # print(r2.send_command("R4 GT 3"))  # Get temperature of heater 3


    R2S.stop_experiment()  # clean start
    R2S.Start()
...
#     R2S.stop_experiment()
    ...
    #simple_flow_sweep(R2S, F0_mL_per_min=1, F1_mL_per_min=0.1, pickup_vol_mL=0.1, reactor_vol_mL=10)
